...
class MyscraperSpider(scrapy.Spider):
    ...
    start_urls = ["https://batdongsan.vn/"]

    start_urls = [
        f'https://batdongsan.vn/ban-nha/p{i}' for i in range(1,45)
    ]
    ...
    def parse(self, response):
        ...
        for dongsan in dongsans:
            ...
            # tieu_de = remove_special_characters(tieu_de) if tieu_de else None
    
            dia_diem = dongsan.css("div.description::text").get()
            dia_diem = dia_diem.replace('\n', '').strip() if dia_diem else None
            # Dùng regex để thay khoảng trắng thừa thành 1 khoảng trắng duy nhất
            ...
            # dien_tich=remove_text(dien_tich) if dien_tich else None
            
            gia = dongsan.css("div.price::text").get()
            gia = gia.replace('\n', '').replace('\t', '').strip() if gia else None
            # gia=xu_ly_gia(gia) if gia else None
    ...
    def parse_details(self, response):
        ...
        # so_phong_ngu=remove_text(so_phong_ngu) if so_phong_ngu else None
        ...
        # so_phong_toilet=remove_text(so_phong_toilet) if so_phong_toilet else None   
        ...
        # mo_ta = remove_special_characters(mo_ta) if mo_ta else None
        # Kết hợp dữ liệu với tên sản phẩm và giá từ meta
        tieu_de = response.meta.get('tieu_de')
        #dia_diem = response.meta.get('dia_diem')
